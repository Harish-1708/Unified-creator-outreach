...
class FakeWs:
    def __init__(self, header=None, col_count=None):
        ...
        # Defaults comfortably wide so existing tests (which don't care
        # about grid limits at all) are unaffected — only a test that
        # explicitly constructs a narrow grid exercises the resize path.
...
def test_get_or_create_ws_grows_narrow_grid_before_writing_new_column():
    """The exact live production crash: 'APIError: [400]: Range
    ('...Master Sheet'!AM1) exceeds grid limits. Max rows: 2000, max
    columns: 38' — a real campaign's Master tab was created back when
    MASTER_COLUMNS had 38 columns; adding AsanaTaskGID and
    ManualAsanaStage since then grew the REQUIRED header past that
    tab's actual grid width, and writing the new header cell without
    growing the grid first raised this exact error, every single run,
    not intermittently — retrying the identical write (as
    _RetryingWorksheet does) could never fix it on its own."""
    narrow_header = [f"col{i}" for i in range(38)]  # the exact reported grid width
    existing_ws = FakeWs(header=list(narrow_header), col_count=38)
    spreadsheet = FakeSpreadsheet(existing={"DudeRobe_Creator_Discovery Master Sheet": existing_ws})
    required = narrow_header + ["NewRequiredColumn"]

    outreach._get_or_create_ws(spreadsheet, FakeGspreadModule,
                                "DudeRobe_Creator_Discovery Master Sheet", required)

    assert existing_ws.resize_calls, "Grid was never resized — this reproduces the exact live crash"
    assert existing_ws.col_count >= 39
    assert existing_ws.row_values(1) == required


def test_get_or_create_ws_does_not_resize_when_grid_already_wide_enough():
    existing_ws = FakeWs(header=["A", "B"], col_count=1000)
    spreadsheet = FakeSpreadsheet(existing={"MyTab": existing_ws})
    outreach._get_or_create_ws(spreadsheet, FakeGspreadModule, "MyTab", ["A", "B", "C"])
...
# =============================================================================
# Adversarial data-type hardening for the send path — gspread types a cell
# by what its content LOOKS like, independent of its column, so any field
# can come back as an int. The send path is the highest-stakes code here;
# a crash mid-run leaves a batch half-sent with no clean way to resume.
# =============================================================================

def test_get_eligible_leads_survives_int_typed_email():
    """The exact crash this prevents: AttributeError: 'int' object has no
    attribute 'strip' — same bug class as the real AsanaTaskGID incident,
    triggered by a numeric-looking value in the Email column."""
    leads = [make_lead(_row=2, LeadID="1", Email=12345, Status="", IntroSentAt="")]
    assert len(outreach.get_eligible_leads(leads, STAGES, 0)) == 1


def test_get_eligible_leads_survives_int_typed_sender_account():
    leads = [make_lead(_row=2, LeadID="1", Email="a@b.com", SenderAccount=999,
                        Status="", IntroSentAt="")]
    assert len(outreach.get_eligible_leads(leads, STAGES, 0)) == 1


def test_get_eligible_leads_stage_index_past_end_returns_empty_not_indexerror():
    """A stale stage name (a stage deleted between workflow dispatch and
    run) must yield nothing eligible, not an IndexError that takes down
    the entire send run for every other lead too."""
    leads = [make_lead(_row=2, LeadID="1", Email="a@b.com", Status="", IntroSentAt="")]
    assert outreach.get_eligible_leads(leads, STAGES, 99) == []


def test_find_duplicate_email_leads_survives_int_typed_emails():
    leads = [make_lead(_row=2, LeadID="1", Email=12345),
             make_lead(_row=3, LeadID="2", Email=12345)]
    duplicates = outreach.find_duplicate_email_leads(leads)
    assert len(duplicates) == 1  # detected as duplicates, no crash


def test_dm_status_map_covers_every_real_dm_status_option():
    """Guards a silent-drift failure: adding a new status to
    shortlist.py's DM_STATUS_OPTIONS without adding it to
    DM_STATUS_TO_ASANA_STAGE would make that status quietly fall back to
    Sourced on every sync — a real creator silently regressing to the
    start of the pipeline, with no error anywhere to notice it by."""
    import sys, os
    discovery_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__)))), "discovery")
    if discovery_dir not in sys.path:
        sys.path.insert(0, discovery_dir)
    import shortlist

    uncovered = [s for s in shortlist.DM_STATUS_OPTIONS
                 if s not in outreach.DM_STATUS_TO_ASANA_STAGE]
    assert uncovered == [], f"DM statuses with no Asana stage mapping: {uncovered}"

    orphans = [k for k in outreach.DM_STATUS_TO_ASANA_STAGE
               if k not in shortlist.DM_STATUS_OPTIONS]
    assert orphans == [], f"Map keys that aren't real DM statuses: {orphans}"


def test_dm_stage_map_only_produces_real_asana_stage_names():
    valid = {outreach.ASANA_STAGE_SOURCED, outreach.ASANA_STAGE_OUTREACH_SENT,
             outreach.ASANA_STAGE_FOLLOWUP, outreach.ASANA_STAGE_NEGOTIATING,
             outreach.ASANA_STAGE_RIGHTS_SECURED, outreach.ASANA_STAGE_DECLINED_DEAD}
    invalid = [(k, v) for k, v in outreach.DM_STATUS_TO_ASANA_STAGE.items() if v not in valid]
    assert invalid == [], f"Map produces stage names Asana won't have: {invalid}"
