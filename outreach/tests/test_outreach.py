...
# =============================================================================
# Asana sync — pure functions
# =============================================================================

def _asana_lead(**overrides):
    lead = {"_row": 2, "Email": "a@abc.com", "Client": "DudeRobe", "Creator": "dudedad",
            "Product": "Robe", "IntroSentAt": "", "FollowUp1SentAt": "", "ReplyStatus": "",
            "AsanaTaskGID": ""}
    lead.update(overrides)
    return lead
...
def test_safe_lead_str_handles_none():
    assert outreach._safe_lead_str(None) == ""


def test_safe_lead_str_handles_int_without_crashing():
    """The actual bug: gspread returns a purely numeric-looking cell
    value as an int, not a str — a raw '.strip()' call on that raises
    AttributeError. Every already-synced lead hit this on the very next
    sync, since only a lead with a real, numeric AsanaTaskGID triggers
    it."""
    assert outreach._safe_lead_str(1218101643744828) == "1218101643744828"


def test_safe_lead_str_strips_whitespace_from_real_strings():
    assert outreach._safe_lead_str("  hello  ") == "hello"


# ---------- compute_lead_asana_stage ----------

def test_stage_sourced_when_nothing_sent():
    assert outreach.compute_lead_asana_stage(_asana_lead()) == outreach.ASANA_STAGE_SOURCED


def test_stage_outreach_sent_when_intro_sent():
    lead = _asana_lead(IntroSentAt="2026-08-01 09:00:00")
    assert outreach.compute_lead_asana_stage(lead) == outreach.ASANA_STAGE_OUTREACH_SENT


def test_stage_follow_up_when_any_followup_sent():
    lead = _asana_lead(IntroSentAt="2026-08-01 09:00:00", FollowUp1SentAt="2026-08-04 09:00:00")
    assert outreach.compute_lead_asana_stage(lead) == outreach.ASANA_STAGE_FOLLOW_UP


def test_stage_negotiating_when_replied_even_with_followups_sent():
    """Checked most-advanced-first: a lead that's both replied AND been
    sent follow-ups must land on Negotiating, not Follow-up."""
    lead = _asana_lead(FollowUp1SentAt="2026-08-04 09:00:00", ReplyStatus="Replied")
    assert outreach.compute_lead_asana_stage(lead) == outreach.ASANA_STAGE_NEGOTIATING


def test_stage_never_returns_a_human_only_stage():
    """No lead data combination should ever compute Rights Secured or
    Declined/Dead — those are always human-set in Asana directly."""
    for lead in [_asana_lead(), _asana_lead(IntroSentAt="x"), _asana_lead(ReplyStatus="Replied")]:
        assert outreach.compute_lead_asana_stage(lead) not in outreach.ASANA_HUMAN_ONLY_STAGES


# ---------- decide_asana_sync_action ----------

def test_decide_action_create_when_no_existing_gid():
    decision = outreach.decide_asana_sync_action(_asana_lead(), current_asana_section_name=None)
    assert decision["action"] == "create"


def test_decide_action_update_when_gid_already_stored():
    lead = _asana_lead(AsanaTaskGID="123456")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Sourced")
    assert decision["action"] == "update"


def test_decide_action_never_moves_out_of_rights_secured():
    """The core safety guarantee: even a lead whose OWN data would
    compute a different stage must not be moved out of Rights Secured —
    it's always human-set, never auto-reverted."""
    lead = _asana_lead(AsanaTaskGID="123456", ReplyStatus="Replied")  # data says Negotiating
...
def test_decide_action_never_moves_out_of_declined_dead():
    lead = _asana_lead(AsanaTaskGID="123456", IntroSentAt="x")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Declined/Dead")
    assert decision["target_stage"] == "Declined/Dead"


def test_decide_action_moves_normally_between_non_human_stages():
    lead = _asana_lead(AsanaTaskGID="123456", ReplyStatus="Replied")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Sourced")
    assert decision["target_stage"] == outreach.ASANA_STAGE_NEGOTIATING


def test_decide_action_explicit_override_forces_create_despite_stale_gid_on_lead():
    """The actual self-healing fix: after a 404 confirms a stored task is
    dead, the caller passes existing_task_gid='' explicitly — this must
    be authoritative over the lead's own (stale) AsanaTaskGID field,
    which still has the dead GID in it at this point."""
    lead = _asana_lead(AsanaTaskGID="dead_gid_still_here")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name=None,
                                                  existing_task_gid="")
    assert decision["action"] == "create"


def test_decide_action_no_override_falls_back_to_lead_field():
    lead = _asana_lead(AsanaTaskGID="123456")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Sourced",
...
def test_task_name_uses_creator_handle_when_present():
    lead = _asana_lead(CreatorHandle="@dudedad")
    assert outreach.build_asana_task_name(lead) == "DudeRobe | @dudedad – Robe"


def test_task_name_falls_back_to_creator_column():
    """The actual bug: a real campaign's Sheet commonly uses 'Creator'
    for the @handle, not a separate 'CreatorHandle' column that never
    existed — without this fallback, EVERY task showed 'DudeRobe |
    DudeRobe' with no indication of which creator it was."""
    lead = _asana_lead()  # has Creator='dudedad', no CreatorHandle
    assert outreach.build_asana_task_name(lead) == "DudeRobe | dudedad – Robe"


def test_task_name_omits_product_dash_when_product_blank():
    lead = _asana_lead(Product="")
    assert outreach.build_asana_task_name(lead) == "DudeRobe | dudedad"


# ---------- build_asana_custom_fields_payload ----------

def _enum_field(name, gid, options):
    return {"name": name, "gid": gid, "type": "enum",
            "enum_options": [{"name": o, "gid": f"{o}_gid"} for o in options]}


def test_custom_fields_maps_by_name_case_insensitive():
    lead = {"Client": "DudeRobe"}
    defs = [{"name": "client", "gid": "field1", "type": "text"}]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert payload == {"field1": "DudeRobe"}


def test_custom_fields_skips_column_with_no_matching_field():
    lead = {"SomeRandomColumn": "value"}
    defs = [{"name": "client", "gid": "field1", "type": "text"}]
    assert outreach.build_asana_custom_fields_payload(lead, defs) == {}


def test_custom_fields_email_fills_creator_email_fallback():
    lead = {"Email": "a@abc.com"}
    defs = [{"name": "Creator Email", "gid": "field1", "type": "text"}]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert payload == {"field1": "a@abc.com"}


def test_custom_fields_does_not_override_explicit_creator_email_column():
    lead = {"Email": "wrong@abc.com", "Creator Email": "right@abc.com"}
    defs = [{"name": "Creator Email", "gid": "field1", "type": "text"}]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert payload == {"field1": "right@abc.com"}


def test_custom_fields_enum_maps_to_option_gid():
    lead = {"Status": "Approved"}
    defs = [_enum_field("Status", "field1", ["Approved", "Rejected"])]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert payload == {"field1": "Approved_gid"}


def test_custom_fields_enum_skips_unmatched_option_rather_than_guessing():
    """An enum value with no matching option on the Asana side must be
    skipped, never guessed at — silently picking the 'closest' option
    risks landing on something the human didn't intend."""
    lead = {"Status": "SomethingNotAnOption"}
    defs = [_enum_field("Status", "field1", ["Approved", "Rejected"])]
    assert outreach.build_asana_custom_fields_payload(lead, defs) == {}


def test_custom_fields_multi_enum_maps_every_matched_value():
    lead = {"Tags": "Fast, Reliable"}
    defs = [{"name": "Tags", "gid": "field1", "type": "multi_enum",
             "enum_options": [{"name": "Fast", "gid": "g1"}, {"name": "Reliable", "gid": "g2"}]}]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert set(payload["field1"]) == {"g1", "g2"}


def test_custom_fields_number_type_converts_to_float():
    lead = {"Score": "8.5"}
    defs = [{"name": "Score", "gid": "field1", "type": "number"}]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert payload == {"field1": 8.5}


def test_custom_fields_skips_field_with_int_gid_from_gspread():
    """A lead field coerced via _safe_lead_str must never crash the
    whole payload build just because gspread typed it as an int."""
    lead = {"Score": 8}
    defs = [{"name": "Score", "gid": "field1", "type": "text"}]
    payload = outreach.build_asana_custom_fields_payload(lead, defs)
    assert payload == {"field1": "8"}


# =============================================================================
# sync_campaign_to_asana — orchestration, mocking _asana_request directly
# (the lowest-level HTTP call) so the 404-vs-403 distinction is tested
# against the real function, not a re-derivation of its logic.
# =============================================================================

class _FakeAsanaSheets:
    def __init__(self, leads):
        self._leads = leads
        self.updates = []
...
def test_sync_self_heals_a_404_stale_task_by_creating_a_replacement(monkeypatch):
    """The core self-healing guarantee: a stored task GID that no longer
    resolves (404 — Asana's own unambiguous 'definitely gone' signal)
    must result in a NEW task being created and the dead GID replaced,
    not a hard failure."""
    lead = _asana_lead(_row=2, AsanaTaskGID="dead_gid")
    sheets = _FakeAsanaSheets([lead])
    ...
    def fake_request(method, path, api_key, **kwargs):
        ...
        if path == "/workspaces":
            return {"data": [{"gid": "ws1"}]}
        if path == "/projects":
            return {"data": [{"gid": "proj1", "name": "TestProject"}]}
        if path == "/projects/proj1/custom_field_settings":
            return {"data": []}
        if path == "/projects/proj1/sections":
            return {"data": [{"gid": "sec1", "name": "Sourced"}]}
        if path == "/tasks/dead_gid":
            raise outreach.AsanaTaskNotFoundError("404")
        if path == "/tasks":
            return {"data": {"gid": "new_task_gid"}}
        if path.startswith("/sections/"):
            return {"data": {}}
        raise AssertionError(f"Unexpected call: {method} {path}")

    monkeypatch.setattr(outreach, "_asana_request", fake_request)
    summary = outreach.sync_campaign_to_asana(sheets, "TestProject", "fake_api_key")

    assert summary["created"] == 1
    assert summary["errors"] == []
    assert sheets.updates == [(2, {"AsanaTaskGID": "new_task_gid"})]


def test_sync_does_not_self_heal_a_403_stays_a_hard_error(monkeypatch):
    """The deliberate asymmetry: a 403 is ambiguous (the task might
    still exist with the token just losing visibility) — this must
    surface as a real, visible error and leave the lead's stored GID
    untouched, never silently create a replacement that risks an
    invisible duplicate."""
    lead = _asana_lead(_row=2, AsanaTaskGID="forbidden_gid")
    sheets = _FakeAsanaSheets([lead])

    class Fake403(requests.exceptions.HTTPError):
        pass
    ...
    def fake_request(method, path, api_key, **kwargs):
        ...
        if path == "/projects/proj1/sections":
            return {"data": [{"gid": "sec1", "name": "Sourced"}]}
        if path == "/tasks/forbidden_gid":
            raise Fake403("403 Forbidden")
        raise AssertionError(f"Unexpected call for the 403 case: {method} {path}")
    ...
    assert "forbidden_gid" in summary["errors"][0]
    assert sheets.updates == []  # never touched — no replacement task, no field change


def test_sync_updates_existing_task_without_recreating_it(monkeypatch):
    """The core no-duplicate-tasks guarantee: a lead with a valid,
    resolvable stored GID must only ever be UPDATED, never re-created."""
    lead = _asana_lead(_row=2, AsanaTaskGID="valid_gid")
    sheets = _FakeAsanaSheets([lead])
    create_called = []
    ...
    def fake_request(method, path, api_key, **kwargs):
        ...
        if path == "/projects/proj1/sections":
            return {"data": [{"gid": "sec1", "name": "Sourced"}]}
        if path == "/tasks/valid_gid" and method == "GET":
            return {"memberships": [{"section": {"name": "Sourced"}}]}
        if path == "/tasks/valid_gid" and method == "PUT":
            return {"data": {}}
        if path == "/tasks" and method == "POST":
            create_called.append(True)
            return {"data": {"gid": "should_not_happen"}}
        if path.startswith("/sections/"):
            return {"data": {}}
        raise AssertionError(f"Unexpected call: {method} {path}")
    ...
    assert sheets.updates == []  # update path never rewrites AsanaTaskGID, it's already correct
