...
MASTER_HEADERS = [
    ...
    # dr_name is the Deep Research report's stated display name (e.g.
    # "Taylor Calmus" for @dudedad) — already extracted, never reached the
    # sheet until now. Needed for the outreach bridge: a lead's FirstName
    # should come from a real display name when one exists, not the raw
    # @handle (which is exactly the display-name/username confusion this
    # whole build already spent real effort fixing at the discovery layer —
    # reintroducing it at the bridge layer would be the same bug twice).
    # Blank for Serper/Gemini-only candidates, same as every other dr_*
    # field — no display name exists for those, only a handle.
    "dr_name",
    # Meta's recent post captions get fetched by the late-stage verification
    # pass either way, but only ever REACH a scoring prompt when
    # SONNET_REFINEMENT=true (the only consumer of the raw "recent_captions"
    # list is classify_creator/refine_creator_with_sonnet, both of which run
    # before verification when refinement is off). With refinement off, they
    # were being fetched and then silently discarded — not shown anywhere,
    # not used anywhere. This makes them visible to a human either way.
    ...
    # The remaining four are also bridge-written, alongside
    # campaign_push_status: which real outreach campaign a push went to
    # (chosen manually at push time, not necessarily the same string as
    # this row's own Campaign — see the bridge's own docstring), the
    # LeadID assigned in that campaign's Master Sheet (the permanent
    # Creator <-> Lead link), when the push happened, and why it failed
    # when it did. All blank until a push is actually attempted.
...
def verify_reported_followers_with_meta(creators: list, access_token: str, ig_business_id: str) -> None:
    ...
    for c in candidates_to_verify:
        ...
        if error is None and data:
            ...
            # Also capture bio/recent captions the same way enrich_instagram's
            # own Meta-success branch does — without this, a candidate could
            # get a "verified" follower count here and still hit Sonnet
            # refinement's "no post captions available, be more conservative"
            # branch a moment later, even though real content evidence was
            # sitting right there in the same API response and simply wasn't
            # kept. Only fills these in if not already present — Serper/
            # Tavily-sourced bio text (if any) isn't overwritten by an empty
            # Meta bio field.
            if data.get("biography") and not c.get("bio"):
                c["bio"] = data.get("biography")
            media_items = (data.get("media") or {}).get("data", [])
            recent_captions = [m["caption"] for m in media_items if m.get("caption")]
            if recent_captions and not c.get("recent_captions"):
                c["recent_captions"] = recent_captions
                # Separate from the raw list above — that one feeds the LLM
                # prompts and must stay a list. This is the human-readable
                # sheet version, always populated when Meta returns captions,
                # regardless of whether SONNET_REFINEMENT is on to actually
                # use them for scoring.
                c["recent_post_captions"] = " | ".join(cap[:120] for cap in recent_captions[:5])
            print(f"[meta_verify] {handle!r}: confirmed {data.get('followers_count')} followers via "
                  f"Meta — upgraded from 'reported' to 'verified'"
                  + (f", {len(recent_captions)} recent caption(s) captured for scoring" if recent_captions else "")
                  + ".")
        else:
...
def deterministic_candidate_check(candidate: dict, extra_exclude_terms: list = None) -> tuple:
    ...
    # A candidate whose only identity is a display name (no confirmed
    # @username — see the Deep Research report parser's handle_type field)
    # can never become an actionable Master row: no real profile_link can be
    # built, Meta/Tavily can't look it up (both already guarded elsewhere
    # against wasting a call on one), and outreach has nothing to link or
    # DM. Scoring it with Claude anyway spends a real API call evaluating a
    # candidate nobody can ever contact. The Deep Research report's own
    # rationale for naming them is preserved in the Excluded row's snippet,
    # not lost — this only stops it from consuming further budget.
    if candidate.get("handle_is_display_name"):
        return False, -100, ("display name only, no confirmed @username — cannot be verified or "
                              "contacted even if the fit looks strong")
...
def run():
    ...
    if cfg["sonnet_refinement"] and final_creators:
        ...
        # Verify HERE, before refinement, not just after — Sonnet's prompt
        # explicitly tells it to be more conservative on content_opportunity
        # when no real post captions are available. A Deep Research candidate
        # normally has none (Meta was skipped since the DR report already had
        # a follower count) — so without this, refinement systematically
        # under-scores exactly the candidates this whole feature exists to
        # verify, using "no captions" language, when Meta could have supplied
        # real captions a few lines earlier if asked. Bounded to pool_size
        # (<= MAX_SONNET_REFINEMENT_POOL candidates) rather than the full
        # final_creators list, for the same cost reasons as before — this
        # does mean more Meta calls than the original post-refinement-only
        # design (up to pool_size instead of result_limit), a deliberate
        # trade for refinement actually having real evidence to work with.
        verify_reported_followers_with_meta(
            final_creators[:pool_size], os.environ.get("META_ACCESS_TOKEN"), os.environ.get("IG_BUSINESS_ACCOUNT_ID"),
        )
    ...
    # Safety-net second pass — idempotent with the pre-refinement call above
    # (only ever touches rows still marked "reported"), so this is cheap when
    # SONNET_REFINEMENT already covered everything. Catches two cases it
    # wouldn't: SONNET_REFINEMENT=false (this becomes the only verification
    # pass, same as the original design), or a candidate that entered the
    # final result_limit rows from outside the refinement pool.
    verify_reported_followers_with_meta(
